...
def job(job_id):
    job = execute_sql('SELECT job.id, job.title, job.description, '
                      'job.salary, employer.id as employer_id, employer.name as employer_name'
                      ' FROM job JOIN employer ON employer.id = job.employer_id WHERE job.id = ?', [job_id], single=True)
...
def employer(employer_id):
    ...
    jobs = execute_sql('SELECT job.id, job.title, job.description, job.salary FROM job JOIN '
                       'employer ON employer.id = job.employer_id WHERE employer.id = ?', [employer_id])
